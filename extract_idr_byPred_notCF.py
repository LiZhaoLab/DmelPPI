...
"""
    Extract IDR by prediction in monomers. Conditionally folding regions are 
    excluded here.
    05/13/2024, by jpeng
"""
...
def longIDR(ss_seqs,ds_scores,lddt_scores,cutoff=20):
    ...
    diso = [0] + [1 if l<=0.7 and d>0.5 else 0 for l,d in zip(lddt_scores,ds_scores)] + [0]
...
f = open("fbpp_longidrs_byPred_noCF.csv","w")
header = "fbpp,longidrs,ss,plddt"
f.write(header+"\n")
pred_cutoff = 5
for fbpp in fbpp2disorder:
    ss,longidrs,idrstrs,plddts = longIDR(*fbpp2disorder[fbpp],cutoff=pred_cutoff)
